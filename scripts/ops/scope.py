    ...
    try:
        ...
        data = {
            ...
            "messages": [
                ...
                {"role": "user", "content": f"Break down this task into an exhaustive, pedantic checklist:\n\n{description}"}
            ],
...
def action_check(index):
    ...
    remaining = [f"{i+1}. {item['text']}" for i, item in enumerate(items) if not item['done']]
    if remaining:
...
def action_status():
    ...
    punch_list = load_punch_list()
    if not punch_list:
        logger.info("No active punch list. Run 'scope.py init <description>' to create one.")
        return True
    ...
    print("\n" + "=" * 70)
    print(f"Progress: {done_count}/{len(items)} items complete ({punch_list['percent']}%)")
    print("=" * 70 + "\n")
...
def main():
    parser = setup_script("The Project Manager: Manages the Definition of Done (DoD) for the current task.")

    # Custom arguments
    parser.add_argument('action', choices=['init', 'check', 'status'],
                       help="Action to perform")
    parser.add_argument('value', nargs='?', help="Value for action (description for init, index for check)")
