...
def main():
    parser = setup_script("The Scanner: Detects available system binaries, languages, and network capabilities")

    parser.add_argument('--compact', action='store_true', help="Show only available tools (hide unavailable)")
    ...
    try:
        ...
        try:
            ...
            if free_gb < 1:
                print(f"   🔴 Disk space: {free_gb:.2f} GB free / {total_gb:.2f} GB total ({used_pct:.1f}% used) - CRITICAL")
            elif free_gb < 5:
                print(f"   ⚠️  Disk space: {free_gb:.2f} GB free / {total_gb:.2f} GB total ({used_pct:.1f}% used) - LOW")
            else:
        ...
        if shutil.which("curl") is None and shutil.which("wget") is None:
            recommendations.append("   🔴 CRITICAL: No HTTP client (curl/wget). Use Python urllib or /dev/tcp")

        if shutil.which("jq") is None:
            recommendations.append("   🟡 WARNING: No jq. Use 'python3 -m json.tool' for JSON parsing")

        if shutil.which("git") is None:
            recommendations.append("   🟡 WARNING: No git. Manual file transfer required")

        if is_root:
            recommendations.append("   ⚠️  NOTICE: Running as root - be extra careful with destructive ops")
