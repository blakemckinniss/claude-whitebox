...
class CodeVisitor(ast.NodeVisitor):
    ...
    def __init__(self, filename, target_type, target_name, show_details, show_decorators):
        self.filename = filename
        self.target_type = target_type
        self.target_name = re.compile(target_name, re.IGNORECASE) if target_name else None
...
def scan_file(filepath, target_type, target_name, show_details, show_decorators):
    ...
    try:
        ...
        visitor = CodeVisitor(filepath, target_type, target_name, show_details, show_decorators)
        visitor.visit(tree)
    ...
    for filepath, file_results in sorted(by_file.items()):
        ...
        for result in file_results:
            ...
            if result_type == 'function':
                decorators_str = f" @{', @'.join(result['decorators'])}" if result['decorators'] else ""
                print(f"  🔹 DEF: {result['name']}({', '.join(result['args'])}){decorators_str}")
                ...
                bases_str = f" (inherits: {', '.join(result['bases'])})" if result['bases'] else ""
                decorators_str = f" @{', @'.join(result['decorators'])}" if result['decorators'] else ""
...
def main():
    parser = setup_script("The X-Ray: Performs AST-based structural search on Python code (Classes, Functions, Imports, Decorators)")

    # Arguments
    parser.add_argument('path', nargs='?', default='.', help="Directory or file to scan (default: current directory)")
    parser.add_argument('--type', dest='search_type', choices=['def', 'function', 'class', 'import', 'call', 'decorator', 'all'],
                       default='all', help="Type of code structure to find")
    parser.add_argument('--name', help="Name pattern to search for (supports regex)")
    parser.add_argument('--details', action='store_true', help="Show docstrings and additional details")
    ...
    try:
        ...
        for filepath in python_files:
            results = scan_file(filepath, args.search_type, args.name, args.details, show_decorators)
            for result in results:
