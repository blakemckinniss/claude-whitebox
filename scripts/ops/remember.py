...
def main():
    parser = setup_script("The Elephant: Manages persistent project memory (Context, Decisions, Lessons)")

    # Arguments
    parser.add_argument('action', choices=['read', 'add', 'search'], default='read', nargs='?',
                       help="Action to perform (read, add, search)")
    parser.add_argument('category', choices=['context', 'decisions', 'lessons', 'all'], default='all', nargs='?',
                       help="Memory category (context, decisions, lessons, all)")
    parser.add_argument('content', nargs='*',
                       help="Content to add (required for 'add' action)")
    ...
    if args.action == 'read':
        ...
        # ADD action
        if args.category == 'all':
            logger.error("Cannot add to 'all' categories. Specify: context, decisions, or lessons")
            finalize(success=False)
        ...
        categories_to_search = [args.category] if args.category != 'all' else ['context', 'decisions', 'lessons']
