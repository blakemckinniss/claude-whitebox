...
CONFIDENCE: 75
REASONING: GraphQL offers flexibility for mobile clients, but requires significant
team training and introduces caching complexity. Performance benefits exist for
nested queries.

INFO_NEEDED:
- Current API call volume (requests/day)
- Team members with GraphQL experience
- ⚠️ CRITICAL: Do we use complex nested data fetching?

BLOCKERS:
- No rate limiting strategy specified
- No query complexity analysis provided

AGREES_WITH: advocate - Flexibility benefits are real
DISAGREES_WITH: critic - 6-month timeline is achievable with proper planning
"""

    # Test case 2: Changed position
    test_output_2 = """
VERDICT: STOP
CONFIDENCE: 85
REASONING: New information shows 45% of queries use relational JOINs. MongoDB
would require complete data model rewrite.

CHANGED_POSITION: CONDITIONAL_GO → STOP
- Reason: JOIN usage makes migration infeasible

AGREES_WITH: judge, skeptic
"""

    # Test case 3: Escalation with recruitment
    test_output_3 = """
VERDICT: ESCALATE
CONFIDENCE: 40
REASONING: This involves cryptographic key rotation which is outside my expertise.

ESCALATE_TO: security - Crypto-specific concerns require specialized review
RECRUITS: legal - GDPR compliance implications detected
"""
    ...
    print("=" * 70)
    result1 = parser.parse(test_output_1, "performance")
    print(f"Parse Success: {result1['parse_success']}")
    print(f"Verdict: {result1['verdict']}")
    print(f"Confidence: {result1['confidence']}")
    print(f"Info Needed: {result1['info_needed']}")
    print(f"Blockers: {result1['blockers']}")
    print(f"Agrees: {result1['agrees_with']}")
    print(f"Errors: {result1['parse_errors']}")
    print(f"Warnings: {result1['parse_warnings']}")

    print("\n" + "=" * 70)
    print("TEST 2: Changed Position")
    print("=" * 70)
    result2 = parser.parse(test_output_2, "skeptic")
    print(f"Parse Success: {result2['parse_success']}")
    print(f"Verdict: {result2['verdict']}")
    print(f"Changed Position: {result2['changed_position']}")
    print(f"Agrees: {result2['agrees_with']}")

    print("\n" + "=" * 70)
    print("TEST 3: Escalation + Recruitment")
    print("=" * 70)
    result3 = parser.parse(test_output_3, "data")
    print(f"Parse Success: {result3['parse_success']}")
    print(f"Verdict: {result3['verdict']}")
    print(f"Escalate To: {result3['escalate_to']}")
    print(f"Recruits: {result3['recruits']}")
