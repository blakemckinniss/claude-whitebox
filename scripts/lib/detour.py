...
# Paths - scripts/lib/detour.py -> scripts/lib -> scripts -> project_root
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: detour_protocol.py [status|test|clear]")
        sys.exit(1)

    cmd = sys.argv[1]

    if cmd == "status":
        print(get_detour_status_report())
        ...
        print("Testing detour detection:\n")
        for output in test_outputs:
            result = detect_detour(output, "Bash", {"command": "test"})
            if result:
                pattern, matched = result
                print(f"✅ Detected: {pattern.detour_type.value}")
                print(f"   Matched: {matched}")
                print(f"   Agent: {pattern.suggested_agent}")
                print(f"   Severity: {pattern.severity}/10")
            else:
                print(f"❌ No detection: {output[:50]}...")
            print()

    elif cmd == "clear":
        if DETOUR_STACK_FILE.exists():
            DETOUR_STACK_FILE.unlink()
            print("Detour stack cleared.")
        else:
            print("No detour stack to clear.")
