...
class InformationGatherer:
    ...
    def _search_metrics(self, request: Dict) -> Dict:
        ...
        if digests_dir.exists():
            # Search digest files for metrics keywords
    ...
    def ask_for_information(missing_requests: List[Dict]) -> Dict[str, str]:
        ...
        for i, req in enumerate(missing_requests, 1):
            ...
            if req["priority"] != "critical":
                response = input(f"   Your answer (or 'skip'): ").strip()
                if response.lower() == "skip":
                    continue
            else:
                response = input(f"   Your answer (REQUIRED): ").strip()
                while not response:
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("TEST: Convergence Detector")
    print("=" * 70)

    detector = ConvergenceDetector(threshold=0.7)

    # Test case 1: No convergence (split verdict)
    test_outputs_1 = [
        {"verdict": "STOP", "persona_name": "judge"},
        {"verdict": "STOP", "persona_name": "skeptic"},
        {"verdict": "CONDITIONAL_GO", "persona_name": "advocate"},
        {"verdict": "PROCEED", "persona_name": "innovator"},
    ]

    result1 = detector.check_convergence(test_outputs_1)
    print(f"\nTest 1 (Split Verdict):")
    print(f"  Converged: {result1['converged']}")
    print(f"  Agreement: {result1['agreement_ratio']*100:.0f}%")
    print(f"  Reason: {result1['reason']}")

    # Test case 2: Convergence (75% agreement, no requests)
    test_outputs_2 = [
        {"verdict": "STOP", "persona_name": "judge", "info_needed": []},
        {"verdict": "STOP", "persona_name": "skeptic", "info_needed": []},
        {"verdict": "STOP", "persona_name": "critic", "info_needed": []},
        {"verdict": "PROCEED", "persona_name": "advocate", "info_needed": []},
    ]

    result2 = detector.check_convergence(test_outputs_2)
    print(f"\nTest 2 (Convergence):")
    print(f"  Converged: {result2['converged']}")
    print(f"  Agreement: {result2['agreement_ratio']*100:.0f}%")
    print(f"  Reason: {result2['reason']}")

    # Test case 3: High agreement but has requests
    test_outputs_3 = [
        {"verdict": "CONDITIONAL_GO", "persona_name": "judge", "info_needed": ["Need metrics"]},
        {"verdict": "CONDITIONAL_GO", "persona_name": "skeptic", "info_needed": []},
        {"verdict": "CONDITIONAL_GO", "persona_name": "performance", "info_needed": []},
    ]

    result3 = detector.check_convergence(test_outputs_3)
    print(f"\nTest 3 (Has Requests):")
    print(f"  Converged: {result3['converged']}")
    print(f"  Agreement: {result3['agreement_ratio']*100:.0f}%")
    print(f"  Reason: {result3['reason']}")

    # Test information gatherer
    print("\n" + "=" * 70)
    print("TEST: Information Gatherer")
    print("=" * 70)

    from pathlib import Path
    project_root = Path.cwd().parent  # Assume running from scratch/
    ...
    test_requests = [
        {
            ...
            "info_needed": [
                ...
                "⚠️ CRITICAL: Team members with GraphQL experience"
            ]
        },
        {
            "persona_name": "data",
            "info_needed": [
                "Do we use JOIN queries in the codebase?"
            ]
        }
    ]

    gathered, missing = gatherer.gather_all_requests(test_requests)

    print(f"\nGathered {len(gathered)} items:")
    for item in gathered:
        print(f"  - {item['request']['description']}")
        print(f"    Source: {item['source']}")

    print(f"\nMissing {len(missing)} items (need user input):")
    for item in missing:
        print(f"  - {item['description']} (requested by {item['requested_by']})")
