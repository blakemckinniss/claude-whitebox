...
const CONTEXT_WINDOW = model.context_window || 200_000;
...
function getGitBranch() {
  try {
    return execSync("git branch --show-current", { encoding: "utf8", timeout: 1000 }).trim();
  } catch {
    return "";
  }
}
...
function getGitStatus() {
  try {
    const status = execSync("git status --porcelain", { encoding: "utf8", timeout: 1000 });
    ...
    const parts = [];
    if (staged > 0) parts.push(`\x1b[32m+${staged}\x1b[0m`);
  ...
  try {
    if (process.platform === "darwin") {
      ...
      if (usagePercent >= 75) {
        ...
        return `${color}RAM ${usagePercent}%\x1b[0m`;
      }
...
function getPythonVenv() {
  ...
  const cwd = process.cwd();
  const commonVenvNames = [".venv", "venv", "env"];
