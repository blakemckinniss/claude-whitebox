...
Solution: Track tool call patterns, suggest scratch/ script after N repetitions
"""
...
REPETITIVE_PATTERNS = {
    "multi_file_edit": {
        "tools": ["Edit", "Write"],
        "threshold": 4,
        "suggestion": "Consider writing a scratch/ script to batch these edits"
    },
    "multi_file_read": {
        "tools": ["Read"],
        "threshold": 5,
        "suggestion": "Use Glob/Grep or write a scratch/ analysis script"
    },
    "multi_bash": {
        "tools": ["Bash"],
        "threshold": 4,
        "suggestion": "Chain commands with && or write a scratch/ script"
    },
    "multi_grep": {
        "tools": ["Grep"],
        "threshold": 4,
        "suggestion": "Write a scratch/ script for complex multi-pattern search"
    }
}
...
def main():
    ...
    if suggestion:
        output = {
            "additionalContext": f"🔄 REPETITIVE PATTERN DETECTED:\n   {suggestion}\n   (scratch/ scripts are faster than manual iteration)"
        }
    else:
        output = {}
