...
def synthesize_batch_results(results):
    """Aggregate batch mode results"""
    print("\n" + "="*70)
    print("📦 BATCH RESULTS")
    print("="*70)
    successful = [r for r in results if r["success"]]
...
def main():
    ...
    # Validate arguments
    if args.generate and not args.query:
        parser.error("--generate requires a query argument")
    if args.test_cases and not args.query:
        parser.error("--test-cases requires a query/target argument")
    if args.batch and not args.query:
        parser.error("--batch requires a query argument")
    ...
    try:
        ...
        if mode == "analyze":
            ...
            synthesize_batch_results(results)
