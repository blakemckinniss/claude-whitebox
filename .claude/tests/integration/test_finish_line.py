#!/usr/bin/env python3
"""
Test Suite for Finish Line Protocol

Tests the functionality of: scripts/ops/scope.py
"""
import unittest
import sys
import os
...
class TestFinishLineProtocol(unittest.TestCase):
    """Tests for Finish Line Protocol"""

    @classmethod
    def setUpClass(cls):
        """Set up test fixtures"""
        cls.project_root = PROJECT_ROOT
        cls.script_path = cls.project_root / "scripts/ops/scope.py"
    ...
    def test_dry_run_mode(self):
        ...
        result = subprocess.run(
            ["python3", str(self.script_path), "status", "--dry-run"],
            capture_output=True,
            text=True,
    ...
    def test_missing_api_key_handling(self):
        ...
        result = subprocess.run(
            ["python3", str(self.script_path), "status"],
            capture_output=True,
            text=True,
            cwd=self.project_root,
            env=env,
